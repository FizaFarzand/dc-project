...
app.add_middleware(
    CORSMiddleware,
    allow_origins=["https://dc-project-gamma.vercel.app"],
...
# ---------------- CREATE ORDER ----------------
@app.post("/orders")
async def create_order(data: OrderCreate, authorization: str = Header(...)):
    db = SessionLocal()
...
# ---------------- GET ORDERS ----------------
@app.get("/orders")
def get_orders(authorization: str = Header(...)):
    db = SessionLocal()
...
def update_order_status(order_id: int, data: OrderUpdate):
    db = SessionLocal()
